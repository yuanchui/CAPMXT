...
void ProcessPendingCommand(void)
{
    ...
    /* Command: "MODE1" - Switch to string mode */
        ...
        MXT_SSN_GetDebug(&in_gap, &no_spi_us, &low_us, &enter_cnt, &exit_cnt);
        if (g_spi_rx_q_head >= g_spi_rx_q_tail) {
            q_depth = (uint16_t)(g_spi_rx_q_head - g_spi_rx_q_tail);
        } else {
            q_depth = (uint16_t)(SPI_RX_QUEUE_DEPTH - g_spi_rx_q_tail + g_spi_rx_q_head);
        }
        snprintf(MXT_WORK_STR, MXT_WORK_BUF_SIZE,
                 ...
                 (unsigned)no_spi_us,
                 (unsigned)low_us,
                 (unsigned long)enter_cnt,
                 (unsigned long)exit_cnt,
                 (unsigned)g_spi_it_active,
                 (unsigned)g_spi_check_requested,
                 (unsigned)g_spi_stream_enabled,
                 (unsigned)q_depth,
                 (unsigned)SPI_RX_QUEUE_DEPTH,
