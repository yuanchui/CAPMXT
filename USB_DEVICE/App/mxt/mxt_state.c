...
uint8_t g_spi_it_rx_buf[2][SPI_IT_CHUNK_LEN];
volatile uint8_t g_spi_it_buf_idx = 0U;
uint8_t g_spi_rx_queue[SPI_RX_QUEUE_DEPTH];
uint8_t g_spi_rx_mark[SPI_RX_QUEUE_DEPTH];
uint32_t g_spi_last_overflow_report_ms = 0;
uint8_t g_spi_tx_buf[SPI_TX_BUF_SIZE];
uint16_t g_spi_tx_len = 0U;
...
uint16_t g_msg_buffer_head = 0;
uint16_t g_msg_buffer_tail = 0;
uint8_t g_msg_buffer_overflow = 0;
