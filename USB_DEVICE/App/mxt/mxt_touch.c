...
static uint8_t MXT_ReadInfoBlock(void)
{
  ...
  if(result == 0) {
    ...
    // 发送信息到USB
        snprintf(MXT_WORK_STR, MXT_WORK_BUF_SIZE, "ID Info: Family=0x%02X, X=%d, Y=%d\r\n", 
...
void MXT_InitTouchScreen(void)
{
  ...
  // 不自动配置SPI/DEBUGCTRL2，由 SPISTART/SPISTOP 手动控制
  // MXT_EnableSPIMode();
}
...
uint8_t MXT_EnableDebugCtrl2(void)
{
  ...
  if (MXT_I2C_Write(g_mxt_i2c_addr, (uint16_t)(g_t6_addr + MXT_T6_DEBUGCTRL2_OFFSET), &debugctrl2, 1) != 0) {
    USB_SendString("ERR: Write DEBUGCTRL2 enable failed\r\n");
    return 2;
  }

  g_debugctrl_applied = 1;
  MXT_SSN_TimStart();
  USB_SendString("INFO: DEBUGCTRL2 enabled (Byte6=0x80 DBGOBJMODEEN)\r\n");
  return 0;
}
...
uint8_t MXT_DisableDebugCtrl2(void)
{
  ...
  if (MXT_I2C_Write(g_mxt_i2c_addr, (uint16_t)(g_t6_addr + MXT_T6_DEBUGCTRL2_OFFSET), &debugctrl2, 1) != 0) {
    USB_SendString("ERR: Write DEBUGCTRL2 disable failed\r\n");
    return 2;
  }

  g_debugctrl_applied = 0;
  if ((g_spi_check_requested == 0U) && (MXT_SSN_StopPullupPending() == 0U)) {
    MXT_SSN_TimStop();
  }
  USB_SendString("INFO: DEBUGCTRL2 disabled (Byte6=0x00)\r\n");
  return 0;
}
