...
void MXT_ExportConfigAsTxt(void)
{
    ...
    if (n_obj > CFG_MAX_OBJECTS) n_obj = CFG_MAX_OBJECTS;
...
uint8_t MXT_PacketUseBridgeBinary(const uint8_t *buf, uint32_t len)
{
  ...
  if (len >= 2U && buf[0] == REPORT_ID && buf[1] == IIC_DATA_1) {
    return 1U;
  }
  if (len >= 3U && buf[0] == IIC_DATA_1) {
    return 1U;
  }
  if (len >= 3U && buf[0] == CMD_CONFIG) {
    return 1U;
  }
  /* 上位机 CFGWRITE 可能在 mode0 文本之前到达（少见） */
  if (buf[0] == CFGWRITE_START_CMD || buf[0] == CFGWRITE_CHUNK_CMD || buf[0] == CFGWRITE_END_CMD) {
    return 1U;
  }
  return 0U;
}
...
void ProcessBridgePacket(uint8_t *buf, uint32_t len)
{
    ...
    /* mode0 或 mxt-app 二进制包 */
    uint8_t as_binary = MXT_PacketUseBridgeBinary(buf, len);
